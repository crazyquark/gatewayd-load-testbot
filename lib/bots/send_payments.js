              ...
              .then(function(recordedPayment) {
                _this._updateBotInstance(recordedPayment)
                  ...
                  .error(function(error) {
                    callback(error);
                    console.log('_updateBotInstanceError', error);
                  });
              })
              .error(function(error) {
                callback(error);
                console.log('_recordTransactionError', error);
              });
          })
          .error(function(error) {
            callback(error);
            console.log('_makePaymentError', error);
          });
      })      
      .error(function(error) {
        callback(error);
        console.log('_buildPaymentError', error);
      });
  },
